...
def get_patch(image, cx, cy, width=128):
    ...
    # rad = width // 2
    # img = np.array(img)
    # patch_img = img[cx - rad: cx + rad, cy - rad: cy + rad]
    #
    # if patch_img.shape[0] < width:
    #     patch_img = np.pad(patch_img, ((0, width - patch_img.shape[0]), (0, 0)), "constant", constant_values=0)
    # if patch_img.shape[1] < width:
    #     patch_img = np.pad(patch_img, ((0, 0), (0, width - patch_img.shape[1])), "constant", constant_values=0)
